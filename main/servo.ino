...
void initServoScales(boundedRangeScaler_t servoScales[]) {
  ...
  servoScalerInitHelper(
    ...
    0.0f, 0.8f, 1.0f // Servo output min, mid, and max. Modify is servo isn't centering or moving to far. Only takes a range of 0.0-1.0
  );

  servoScalerInitHelper(
    servoScales[SERVO_FRONT_RIGHT], 
    35.0f, -90.0f, // Servo control surface angle min and max, use a higher then lower number to swap directions
  ...
  servoScalerInitHelper(
    ...
    0.0f, 0.65f, 1.0f // Servo output min, mid, and max. Modify is servo isn't centering or moving to far. Only takes a range of 0.0-1.0
  );
